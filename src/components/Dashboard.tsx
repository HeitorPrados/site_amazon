...
export default function Dashboard({ userEmail, onLogout }: DashboardProps) {
  ...
  return (
    <div id="appShell">
      <header className="header-bg p-[26px_30px_20px] border-b border-line-strong flex justify-between items-end flex-wrap gap-4">
        <div>
        ...
        <div className="flex flex-col items-end gap-2.5">
          ...
          <div className="flex border border-line-strong bg-panel font-mono">
            <div className="p-[9px_16px] border-r border-line-strong min-w-[100px]">
              <div className="font-display text-[19px] font-bold text-white">{stats.total}</div>
              <div className="font-mono text-[9.5px] uppercase tracking-[.08em] text-muted mt-0.5">produtos</div>
            </div>
            <div className="p-[9px_16px] border-r border-line-strong min-w-[100px]">
              <div className="font-display text-[19px] font-bold text-white">{stats.filled}</div>
              <div className="font-mono text-[9.5px] uppercase tracking-[.08em] text-muted mt-0.5">c/ margem</div>
            </div>
            <div className="p-[9px_16px] border-r border-line-strong min-w-[100px]">
              <div className="font-display text-[19px] font-bold text-white">{stats.pos}</div>
              <div className="font-mono text-[9.5px] uppercase tracking-[.08em] text-muted mt-0.5">margem +</div>
            </div>
            <div className="p-[9px_16px] min-w-[100px]">
              <div className="font-display text-[19px] font-bold text-white">{stats.avg}</div>
              <div className="font-mono text-[9.5px] uppercase tracking-[.08em] text-muted mt-0.5">margem média</div>
            </div>
          </div>
        </div>
      </header>

      <div className="p-[7px_30px] bg-[#0e1117] border-b border-line flex items-center gap-2 font-sans text-[10.5px] text-muted2">
      ...
      <main className="p-[20px_30px_60px]">
        <div className="flex gap-2 items-center mb-3.5 flex-wrap">
          <input
            ...
            className="flex-1 min-w-[220px] p-[9px_12px] border border-line-strong bg-panel text-ink font-mono text-[12.5px] focus:outline-none focus:border-amber placeholder:text-muted2"
          />
          <select
            value={filterVal}
            onChange={e => setFilterVal(e.target.value)}
            className="p-[9px_10px] border border-line-strong bg-panel text-ink font-sans text-[12.5px] focus:outline-none"
          >
          ...
          <div className="flex flex-1 gap-2 min-w-[220px]">
            ...
            <button
              ...
              disabled={!newAsin}
              className="p-[6px_10px] text-[10.5px] border border-line-strong bg-transparent text-muted font-mono font-semibold tracking-[.03em] uppercase cursor-pointer transition-colors hover:text-ink hover:border-ink disabled:opacity-50"
            >
              + adicionar
            </button>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="border border-line-strong bg-panel overflow-x-auto">
            <table className="w-full border-collapse min-w-[980px]">
              ...
              <tbody>
                {filteredProducts.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-0 border-r border-line last:border-r-0">
